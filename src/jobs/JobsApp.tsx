...
import { writeTailorHandoff } from "../lib/tailor-handoff.ts";
...
export default function JobsApp() {
  ...
  // consumes the handoff on mount, sets `jdContext`, and switches to the
  // Reconstructed tab. `buildJdRewriteContext` returns null when there is
  // nothing useful to steer with (e.g. every JD term is already covered), in
  // which case we don't navigate — a no-op leaves the user on the posting
  // instead of shipping them home for a generic rewrite prompt.
  const handleTailor = (coverage: CoverageResult) => {
    const jdContext = buildJdRewriteContext(coverage);
    if (jdContext === null) return;
    writeTailorHandoff({ jdContext });
