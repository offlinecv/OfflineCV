 ...
 *    sees arrive;
 *  - eligibility is per parse (#1005): a new résumé is asked about, the same
 *    one never twice — across a remount too — and an ask earned by one résumé
 *    never surfaces on the next;
 *  - finishing Fix It is a milestone of its own, shown at once;
 *  - the global cooldown is elapsed time, not a lifetime cap: inside the window
 *    the ask is suppressed, past it the ask returns, and a future-dated stamp
...
interface Harness {
  ...
  /** Put a different résumé on screen (or none). */
  setParse: (key: string | null) => Promise<void>;
}

async function mount(parseKey: string | null = "parse-a"): Promise<Harness> {
  vi.resetModules();
  ...
  const render = async (k: string | null) => {
    await act(async () => {
      root!.render(createElement(Probe, { k }));
    });
  };
  await render(parseKey);

  return { api: () => current!, setParse: render };
}
...
describe("useFeedbackDialog", () => {
  ...
  it("a star on the nudge opens the dialog on that rating and retires the nudge", async () => {
    ...
    await act(async () => h.api().rateFromNudge(2));
  ...
  it("the ambient open ignores its argument — a Button's onClick passes the click event", async () => {
    // `ParsedHeader` and `PageShell` hand `openDialog` straight to a `Button`
    // `onClick`, which spreads onto `<button>` and calls it with the event. When
    // the open took an optional rating, that event became `initialRating`.
    const h = await mount();
    const asOnClick = h.api().openDialog as (event: unknown) => void;
    await act(async () => asOnClick({ type: "click" }));
    expect(h.api().open).toBe(true);
    expect(h.api().initialRating).toBe(0);
  });
  ...
  it("suppresses the ask inside the cooldown window, even for a new résumé", async () => {
    window.localStorage.setItem(
      "ocv_feedback_prompted_at",
      String(Date.now() - 5 * MINUTE_MS),
    );
    const h = await mount();
    await exportResume(h);
    expect(h.api().nudgeVisible).toBe(false);
  ...
  it("does not let a future-dated timestamp hold the ask off", async () => {
    // A clock set back, or a hand-edited key: without the guard this waits
    // until that date, which may be never.
    window.localStorage.setItem(
      "ocv_feedback_prompted_at",
      String(Date.now() + 365 * DAY_MS),
    );
    const h = await mount();
    await exportResume(h);
    expect(h.api().nudgeVisible).toBe(true);
  ...
  it("an ask earned by one résumé does not surface on the next", async () => {
    // Export résumé A, ignore the nudge, drop résumé B: the nudge is about A.
    const h = await mount("parse-a");
    await exportResume(h);
    expect(h.api().nudgeVisible).toBe(true);

    await h.setParse(null); // B parsing
    expect(h.api().nudgeVisible).toBe(false);
    await h.setParse("parse-b");
    expect(h.api().nudgeVisible).toBe(false);
  });

  it("an export armed on one résumé does not flush onto the next", async () => {
    const h = await mount("parse-a");
    await act(async () => h.api().notifyResumeExported());
    await h.setParse("parse-b");
    await act(async () => h.api().notifyExportClosed());
  ...
  it("a new résumé is eligible for its own ask once the cooldown allows", async () => {
    const h = await mount("parse-a");
    await exportResume(h);
    await act(async () => h.api().dismissNudge());
    // Past the global cooldown — what is being tested is that A's record does
    // not cover B.
    window.localStorage.setItem(
      "ocv_feedback_prompted_at",
      String(Date.now() - DAY_MS),
    );
    const h2 = await mount("parse-b");
    await exportResume(h2);
    expect(h2.api().nudgeVisible).toBe(true);
  });

  it("never asks about the same résumé twice, even across a reload", async () => {
    const h = await mount("parse-a");
    await exportResume(h);
    await act(async () => h.api().dismissNudge());
    window.localStorage.setItem(
    ...
    const again = await mount("parse-a");
    await exportResume(again);
    expect(again.api().nudgeVisible).toBe(false);
    await act(async () => again.api().notifyFixItFinished());
    expect(again.api().nudgeVisible).toBe(false);
  });

  it("finishing Fix It raises the nudge at once, and never the dialog", async () => {
    const h = await mount();
    await act(async () => h.api().notifyFixItFinished());
    expect(h.api().nudgeVisible).toBe(true);
    expect(h.api().open).toBe(false);
  });

  it("Fix It and an export on the same résumé ask once between them", async () => {
    const h = await mount();
    await act(async () => h.api().notifyResumeExported());
    await act(async () => h.api().notifyFixItFinished());
    await act(async () => h.api().dismissNudge());
  ...
  it("with no parsed résumé on screen, nothing is ever asked", async () => {
  ...
  it("tracks whether a Fix It dock is hosting the nudge", async () => {
    const h = await mount();
    expect(h.api().nudgeHosted).toBe(false);
    let unregister: () => void = () => {};
    await act(async () => {
      unregister = h.api().registerNudgeHost();
    });
    expect(h.api().nudgeHosted).toBe(true);
    await act(async () => unregister());
    expect(h.api().nudgeHosted).toBe(false);
  });

  it("an unreadable asked-about list reads as empty", async () => {
    window.localStorage.setItem("ocv_feedback_asked", "{not json");
    const h = await mount();
    await exportResume(h);
    expect(h.api().nudgeVisible).toBe(true);
  });
