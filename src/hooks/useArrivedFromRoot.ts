 ...
 * `/jobs/` needs to know whether this document's arrival is the far end of
 * that trip. The marker records only WHERE the trip started, never where it
 * was headed, so the surface that ANSWERS must also be the surface that
 * retires it: consumed at click time instead, a marker could survive its
 * intended leg and answer the next back control the user happens to reach —
 * the two-hop defect a retired second surface historically triggered.
 *
 ...
 * StrictMode's setup→cleanup→setup effect replay is a no-op.
 */
