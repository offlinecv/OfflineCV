...
/**
 * #767 review, secondary: the company offer used to ALWAYS insert, so with the
 * company's own letter on screen it offered to fork that letter from itself.
 * The duplicate was then unreachable forever — the chain surfaces only the
 * most-recently-updated record per rung, it has no `jobId` for
 * `deleteLettersForJob` to cascade to, and there is no company-letter list to
 * delete it from.
 */
describe("JobLetterIndicator company tier: edit, not fork (#767 review)", () => {
  ...
  it("still forks when the letter on screen belongs to another scope", () => {
    // Lifting one job's draft to company scope is the offer's original job, and
    // it must stay an insert — that is a new record at a new scope.
    dom.render(
      <JobLetterIndicator
        jobId="job-1"
        companyKey="northwind"
