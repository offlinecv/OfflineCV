...
describe("JobResultRefineStrip (issue 809)", () => {
  ...
  it("adds an exclude term through the same onChange", () => {
    ...
    const input = el.querySelector<HTMLInputElement>(
      'input[aria-label="Add exclude term (narrow results)"]',
    );
  ...
  it("suffixes the headings the form also has with where they sit (#905 review)", () => {
    const { el } = render(baseQuery);
    const headings = [...el.querySelectorAll("h3")].map((h) => h.textContent);
    expect(headings).toEqual([
      "Narrow these results",
      "Level (narrow results)",
      "Exclude (narrow results)",
    ]);
    // The suffix is for the accessibility tree only — the visible text stays.
    // The first heading is already unique to this surface and takes none.
    for (const h of [...el.querySelectorAll("h3")].slice(1)) {
      expect(h.querySelector(".sr-only")?.textContent).toBe(" (narrow results)");
    }
  });
});
