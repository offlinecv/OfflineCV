 ...
 *
 * Both surfaces can be mounted at once (the form's Narrow step stays mounted
 * while inactive, and "Edit search" opens it over a loaded result set), so the
 * optional `context` is appended to every accessible name — "Location (narrow
 * results)" — or a screen reader hears "Location, edit" twice with nothing
 * telling them apart, and a panel-level `getByLabelText` is ambiguous (#905
 * review). The form passes nothing and keeps its original names.
 */
...
import { accessibleName } from "./QueryStepSection.tsx";
...
interface FieldProps {
  ...
  /** Where this instance sits, for the accessible name only — see the docblock. */
  context?: string;
}

export function LocationField({ query, onChange, context }: FieldProps) {
  return (
    <EditableField
      value={query.location}
      placeholder="location"
      label={accessibleName("Location", context)}
      onCommit={(v) => onChange((q) => ({ ...q, location: v || undefined }))}
    />
  );
}

export function ExcludeTermsEditor({ query, onChange, context }: FieldProps) {
  return (
    <ChipListEditor
      label={accessibleName("Excluded titles", context)}
      labelHidden
      ...
      addAriaLabel={accessibleName("Add exclude term", context)}
    />
  );
}
