 ...
 * for that skill. The canonical ID is a stable key; the authored `label` is
 * what the UI renders, and aliases are what we phrase-match against the JD and
 * the resume corpus.
 *
 * Constraints:
...
export interface SkillEntry {
  ...
  /** Human display form, authored for every entry so stable IDs never leak
   *  into the UI. Keep the project's brand casing (`iOS`, `P&L Ownership`,
   *  `A/B Testing`) rather than deriving it mechanically from the ID.
   *
   *  Must start with a capital or digit unless it is an explicitly allowlisted
   *  lowercase-starting brand — `skills.test.ts` enforces this.
 ...
 * - 1.2 (2026-08-16): authored display labels for every dictionary entry,
 *   including brand casing for previously implicit labels (#681).
 */
...
export const SKILLS: readonly SkillEntry[] = [
  ...
  { id: "websocket", label: "WebSockets", aliases: ["websocket", "websockets"] },
  ...
  { id: "airflow", label: "Apache Airflow", aliases: ["airflow", "apache airflow"] },
  { id: "spark", label: "Apache Spark", aliases: ["spark", "apache spark"] },
