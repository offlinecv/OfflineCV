...
describe("runCascadeFromMarkdown — real-world DOCX fixtures", () => {
  ...
  // Fixture mirrors the shape of `Resume of Srinivas Annam.docx` that hit
  ...
  const SPLIT_LETTER_HEADER_DOCX = [
    ...
    "E **408-372-6626**",
    "",
    " [**annam@annam.org**](mailto:annam@annam.org)",
    "",
    "e **Santa Clara, CA**",
    "",
    "**S UMMARY**",
  ...
  it("the previously-0.0 DOCX now extracts experience and clears basics", async () => {
    ...
    expect(result.parsed.full_name).toBe("Srinivas Annam");
    expect(result.parsed.email).toBe("annam@annam.org");
    expect(result.parsed.phone).toMatch(/408-372-6626/);
...
describe("runCascadeFromMarkdown — two-column DOCX with sidebar bleedthrough", () => {
  // Faithful slim of the `Resume of Srinivas Annam.docx` shape that
