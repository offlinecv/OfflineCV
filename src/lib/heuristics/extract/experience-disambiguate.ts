...
// Exported for one purpose: `corporate-suffix.test.ts` pins its `.source` and
// `.flags` byte-identical to the pre-#917 literal. Composing it from a key list
// moves the drift risk from "someone retypes the regex" to "someone edits the
// key list", and only a golden on THIS constant catches the second.
export const LEGAL_SUFFIX_RE = composeSuffixRegex(
  selectSuffixTokens(
...
// Exported for the byte-identity golden — see `LEGAL_SUFFIX_RE` above.
export const COMPANY_TAIL_TOKENS_RE = composeSuffixRegex(
  selectSuffixTokens([
...
// Exported for the byte-identity golden — see `LEGAL_SUFFIX_RE` above.
export const COMPANY_LEGAL_TAIL_RE = composeSuffixRegex(
  selectSuffixTokens([
