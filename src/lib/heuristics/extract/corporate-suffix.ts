...
export function composeSuffixRegex(
  ...
  // The array form matches by exact string, so an entry that is not in
  // `tokens` contributes nothing and says nothing — the set silently loses its
  // #641 dot tolerance. The live trap is `lowercase`: `LEGAL_SUFFIX_RE` passes
  // it to BOTH `selectSuffixTokens` calls, and omitting it on the
  // `allowTrailingDot` one makes every entry miss at once, so "Acme Inc."
  // stops matching with no compile error and no failing mechanics test.
  // Rejected at construction, like a stateful flag below: selecting by key
  // makes a mis-spelled TOKEN a compile error, and this is what gives the dot
  // subset — which is typed `readonly string[]`, not by key — the same
  // guarantee.
  if (dotSubset) {
    const known = new Set(tokens);
    for (const token of dotSubset) {
      if (!known.has(token)) {
        throw new Error(
          `composeSuffixRegex: allowTrailingDot names "${token}", which is not in ` +
            "tokens — its dot would be silently dropped (check that the same " +
            "`lowercase` option was passed to both selectSuffixTokens calls)",
        );
      }
    }
  }
