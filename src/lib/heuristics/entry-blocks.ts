 ...
 *  ({@link MIDDOT}, not the spaced `MIDDOT_JOIN` the exporter composes with —
 *  that one is not imported here, so it is named rather than linked) is robust to
 *  spacing collapse on re-extraction, which is exactly why the two spellings both
 *  live in the shared contract module (#649) instead of being re-typed at each
 *  end. */
