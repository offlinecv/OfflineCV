...
// `ComposedRoleHeader` is likewise not re-exported: callers of
// `composeRoleHeader` read `headerLine`/`subLine` off the returned object and
// never name its type, so the name crosses no seam either.
export type { RoleHeaderFields } from "./role-header.ts";
