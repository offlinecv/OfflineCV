...
/**
 * The letters at the company and standard tiers that NO resolution chain can
 * reach (#978) — every record but the one {@link mostRecent} would answer with.
 *
 * The complement of the chain, and it lives here for that reason: the two must
 * agree about which record is reachable, and a second definition of "most
 * recent" beside a surface's markup is how they would come to disagree. A
 * record this returns is one the user authored and that no view in the app can
 * show — so it is also the one that cannot be deleted without being listed
 * first.
 *
 * The job tier is deliberately absent. `lettersForJob` already returns every
 * draft for a job and the reveal lists all of them, so nothing there is
 * unreachable; it is the two keyless tiers, which surface one record each, that
 * lose duplicates.
 *
 * Both lists are most-recently-updated first and are ordinarily EMPTY — a store
 * holding at most one record per tier yields nothing here, which is what keeps
 * every surface unchanged for it.
 *
 * `companyKey` omitted skips the company tier entirely, matching
 * {@link inheritedLetterForJob}'s treatment of a job with no company name.
 */
export function unreachableLetters(
  letters: readonly LetterRecord[],
  companyKey?: string,
): { company: LetterRecord[]; standard: LetterRecord[] } {
  return {
    company:
      companyKey === undefined
        ? []
        : behindTheHead(letters, (letter) => isCompanyLetter(letter, companyKey)),
    standard: behindTheHead(
      letters,
      (letter) => letter.jobId === undefined && letter.companyKey === undefined,
    ),
  };
...
/** Everything a tier holds except the record `mostRecent` answers with.
 *
 *  `sort` is stable and `mostRecent` keeps the earlier element on a tie, so
 *  `[0]` here is the same record the chain picks and `slice(1)` is exactly the
 *  unreachable set — the equality this whole function rests on. */
function behindTheHead(
  letters: readonly LetterRecord[],
