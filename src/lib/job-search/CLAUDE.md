...
call this app does not make. Its only vocabulary is borrowed from `pdf/country-registry.ts`
(US states, country names) so "TX" / "Texas" / "USA" / "United States" fold together and a
remote-only board's eligibility region ("USA, Canada") can be placed; there is no gazetteer,
and the limits that leaves are listed on `locationMatches`.
